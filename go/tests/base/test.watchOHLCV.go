package base
import "github.com/ccxt/ccxt/go/v4"

// PLEASE DO NOT EDIT THIS FILE, IT IS GENERATED AND WILL BE OVERWRITTEN:
// https://github.com/ccxt/ccxt/blob/master/CONTRIBUTING.md#how-to-contribute-code


    func TestWatchOHLCV(exchange ccxt.ICoreExchange, skippedProperties interface{}, symbol interface{}) <- chan interface{} {
                ch := make(chan interface{})
                go func() interface{} {
                    defer close(ch)
                    defer ReturnPanicError(ch)
                        var method interface{} = "watchOHLCV"
                var now interface{} = exchange.Milliseconds()
                var ends interface{} = Add(now, 15000)
                var timeframeKeys interface{} = ObjectKeys(exchange.GetTimeframes())
                Assert(GetArrayLength(timeframeKeys), Add(Add(Add(exchange.GetId(), " "), method), " - no timeframes found"))
                // prefer 1m timeframe if available, otherwise return the first one
                var chosenTimeframeKey interface{} = "1m"
                if !IsTrue(exchange.InArray(chosenTimeframeKey, timeframeKeys)) {
                    chosenTimeframeKey = GetValue(timeframeKeys, 0)
                }
                var limit interface{} = 10
                var duration interface{} = exchange.ParseTimeframe(chosenTimeframeKey)
                var since interface{} = Subtract(Subtract(exchange.Milliseconds(), Multiply(Multiply(duration, limit), 1000)), 1000)
                var consumer interface{} =     func(message *ccxt.Message)  {
                    if IsTrue(message.Error) {
                        panic(ExchangeError(message.Error))
                    }
                    if !IsTrue(message.Payload) {
                        panic(ExchangeError("received null or undefined payload"))
                    }
                }
                
                    {
                         func() (ret_ interface{}) {
                		    defer func() {
                                if e := recover(); e != nil {
                                    if e == "break" {
                                        return
                                    }
                                    ret_ = func() interface{} {
                                        // catch block:
                                                if !IsTrue(IsTemporaryFailure(e)) {
                            panic(e)
                        }
                                        return nil
                                    }()
                                }
                            }()
                		    // try block:
                            
                        retRes308 := (<-exchange.SubscribeOHLCV(symbol, chosenTimeframeKey, consumer, true))
                        PanicOnError(retRes308)
                		    return nil
                	    }()
                    
                        }
                for IsLessThan(now, ends) {
                    var response interface{} = nil
                    var success interface{} = true
                    
                        {
                             func() (ret_ interface{}) {
                    		    defer func() {
                                    if e := recover(); e != nil {
                                        if e == "break" {
                                            return
                                        }
                                        ret_ = func() interface{} {
                                            // catch block:
                                                        if !IsTrue(IsTemporaryFailure(e)) {
                                    panic(e)
                                }
                                now = exchange.Milliseconds()
                                // continue;
                                success = false
                                            return nil
                                        }()
                                    }
                                }()
                    		    // try block:
                                            
                        response = (UnWrapType(<-exchange.WatchOHLCV(symbol, chosenTimeframeKey, since, limit)))
                                    PanicOnError(response)
                    		    return nil
                    	    }()
                        
                            }
                    if IsTrue(IsEqual(success, true)) {
                        AssertNonEmtpyArray(exchange, skippedProperties, method, response, symbol)
                        now = exchange.Milliseconds()
                        for i := 0; IsLessThan(i, GetArrayLength(response)); i++ {
                            TestOHLCV(exchange, skippedProperties, method, GetValue(response, i), symbol, now)
                        }
                    }
                }
            
                ch <- true
                return nil
            
                }()
                return ch
            }
