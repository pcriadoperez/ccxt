...
class OrderRouter {
    ...
    static { this.DEFAULT_BASE_URL = 'https://docs.ccxt.com/router/api'; }
     ...
     */
    numberAt(container, key, defaultValue) {
     ...
     */
    stringAt(container, key, defaultValue) {
        if (container === undefined || container === null) {
            return defaultValue;
        }
        const value = container[key];
     ...
     */
    boolAt(container, key, defaultValue) {
        if (container === undefined || container === null) {
            return defaultValue;
        }
        const value = container[key];
     ...
     */
    assertRouteChainIsCoherent(route, hops) {
        if (hops.length === 0) {
            return;
        }
        let carried = '';
     ...
     */
    buildExecutionPlan(route, options = {}) {
     ...
     */
    violation(stepIndex, exchangeId, symbol, code, blocking, actual, limit) {
        return {
            'stepIndex': stepIndex,
     ...
     */
    notionalUsd(step, notionalQuote, usdRates) {
     ...
     */
    precisionViolated(value, precision, mode) {
        if (precision <= 0) {
     ...
     */
    reconcileExecutionStep(plan, stepIndex, realisedOut) {
        const steps = this.listAt(plan, 'steps');
     ...
     * @returns {undefined}
     */
    addPosition(positions, exchangeId, asset, amount, source, produced) {
        for (let i = 0; i < positions.length; i++) {
            const position = positions[i];
     ...
     */
    emptyReport(plan, strategy, requestedStrategy, live, steps) {
        const results = [];
        for (let i = 0; i < steps.length; i++) {
            const step = steps[i];
            results.push({
     ...
     * @returns {undefined}
     */
    async executeSequential(report, steps, venues, options, usdRates, strategy) {
        const results = this.listAt(report, 'steps');
        for (let i = 0; i < steps.length; i++) {
            const step = steps[i];
     ...
     * @returns {undefined}
     */
    async executeParallelWithinHop(report, steps, venues, options, usdRates) {
        const results = this.listAt(report, 'steps');
        let cursor = 0;
     ...
     * @returns {undefined}
     */
    async placeVenueGroup(indices, steps, venues, options, usdRates, report, results) {
     ...
     * @returns {undefined}
     */
    async executeBestEffort(report, steps, venues, options, usdRates) {
        const results = this.listAt(report, 'steps');
     ...
     */
    async placeStep(step, venues, options, usdRates, strategy, report) {
     ...
     * @returns {undefined}
     */
    recordUnconfirmedPlacement(report, exchangeId, symbol, reason) {
     ...
     * @returns {undefined}
     */
    recordOpenOrder(report, exchangeId, symbol, orderId, reason) {
        if (orderId === '') {
     ...
     */
    async refetchOrder(venue, orderId, symbol, fallback) {
        try {
     ...
     */
    async placeImmediateOrder(venue, symbol, side, amount, price, orderParams, options, result) {
     ...
     */
    async placeProtectedLimit(venue, step, symbol, side, amount, price, orderParams, options, report, result) {
     ...
     * @returns {undefined}
     */
    applyResize(steps, reconciliation) {
     ...
     * @returns {undefined}
     */
    markRemainingSkipped(results, start) {
     ...
     * @returns {undefined}
     */
    recordError(report, stepIndex, exchangeId, symbol, code) {
