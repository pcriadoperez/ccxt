...
// Every numeric setting goes through this rather than a bare Number(). An unparseable value used
// to become NaN and flow onwards silently: NaN fails every comparison, so a NaN staleness cutoff
// makes no book stale, a NaN penalty makes every score NaN, path ranking loses its ordering, and
// the caller gets a 200 whose amounts are all null with unroutableReason still null. A wrong
// answer that looks like a right one is the worst thing this service can produce, and a typo in a
// unit file should not be able to cause it — so refuse to start instead.
...
export const config = {
    ...
    minExchangesPerSymbol: numberFromEnv('ORDER_ROUTER_MIN_EXCHANGES_PER_SYMBOL', 2),
    ...
    topSymbols: numberFromEnv('ORDER_ROUTER_TOP_SYMBOLS', 50),
    ...
    // DEFAULT 0 (off) for the beta. When it fires it stops ALL shards, including ones whose
    // assignment did not change, so every book ages past staleBookMs and /route answers
    // all_books_stale for the minutes the cache takes to rewarm — two minutes after every boot,
    // which is also when a deploy's smoke test has just passed and its live-integration job is
    // about to run. Re-enable only alongside a rolling restart that replaces one shard at a time
    // and waits for the replacement to warm.
    rebalanceAfterMs: numberFromEnv('ORDER_ROUTER_REBALANCE_AFTER_MS', 0),
    rebalanceMinImbalance: numberFromEnv('ORDER_ROUTER_REBALANCE_MIN_IMBALANCE', 1.5),
    ...
    loadMarketsConcurrency: numberFromEnv('ORDER_ROUTER_LOAD_MARKETS_CONCURRENCY', 8),
    ...
    maxSymbolsPerSubscription: numberFromEnv('ORDER_ROUTER_MAX_SYMBOLS_PER_SUBSCRIPTION', 50),
    ...
    shardCount: numberFromEnv('ORDER_ROUTER_SHARD_COUNT', 1),
    ...
    stalenessPenaltyBps: numberFromEnv('ORDER_ROUTER_STALENESS_PENALTY_BPS', 1),
    ...
    hopPenaltyBps: numberFromEnv('ORDER_ROUTER_HOP_PENALTY_BPS', 5),
    ...
    wsMinPushIntervalMs: numberFromEnv('ORDER_ROUTER_WS_MIN_PUSH_INTERVAL_MS', 100),
    ...
    keysReloadPollMs: numberFromEnv('ORDER_ROUTER_KEYS_RELOAD_POLL_MS', 10_000),
    ...
    shardMaxOldSpaceMb: numberFromEnv('ORDER_ROUTER_SHARD_MAX_OLD_SPACE_MB', 1024),
    ...
    shardStartConcurrency: numberFromEnv('ORDER_ROUTER_SHARD_START_CONCURRENCY', 2),
    ...
    maxBookDepth: numberFromEnv('ORDER_ROUTER_MAX_BOOK_DEPTH', 500),
    ...
    databasePoolMax: numberFromEnv('ORDER_ROUTER_DB_POOL_MAX', 10),
    ...
    ingestIntervalMs: numberFromEnv('ORDER_ROUTER_INGEST_INTERVAL_MS', 5000),
    ...
    keyProjectionIntervalMs: numberFromEnv('ORDER_ROUTER_KEY_PROJECTION_INTERVAL_MS', 5000),
    ...
    webPort: numberFromEnv('ORDER_ROUTER_WEB_PORT', 8090),
    ...
    rateLimitMax: numberFromEnv('ORDER_ROUTER_RATE_LIMIT_MAX', 600),
    rateLimitWindowMs: numberFromEnv('ORDER_ROUTER_RATE_LIMIT_WINDOW_MS', 60_000),
    ...
    wsMaxConnectionsPerKey: numberFromEnv('ORDER_ROUTER_WS_MAX_CONNECTIONS_PER_KEY', 50),
    ...
    wsIdleTimeoutMs: numberFromEnv('ORDER_ROUTER_WS_IDLE_TIMEOUT_MS', 30_000),
