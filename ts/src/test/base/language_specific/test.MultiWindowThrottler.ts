...
import {
    ...
    RateLimitConfig,
} from '../../../base/functions/MultiWindowThrottler.js';
 ...
 */
async function testBasicWindowLimiter(): Promise<void> {
 ...
 */
async function testWeightBasedLimiting(): Promise<void> {
 ...
 */
async function testMultipleParallelLimiters(): Promise<void> {
 ...
 */
async function testDynamicLimitUpdates(): Promise<void> {
 ...
 */
async function testSlidingWindow(): Promise<void> {
 ...
 */
async function testMultipleIntervals(): Promise<void> {
 ...
 */
async function testBinanceLikeScenario(): Promise<void> {
    ...
    // Simulate different endpoint costs - await all sequentially
    await throttler.throttle(1);  // Lightweight endpoint (weight 1)
    await throttler.throttle(5);  // Medium endpoint (weight 5)
    await throttler.throttle(100);  // Heavy endpoint (weight 100)

    // Small delay to ensure all requests are recorded
...
async function testQueueOverflow(): Promise<void> {
    ...
    const throttler = new MultiWindowThrottler({
        rateLimits: [
            { limit: 1, interval: 'second' }, // Allow 1 per second
        ],
        maxCapacity: 5, // Small queue for easier testing
    });

    // Fill the queue completely
    // First request goes through immediately (starts at full capacity)
    // Requests 2-6 fill the queue (5 items)
    const promises: Promise<void>[] = [];
    for (let i = 0; i < 6; i++) {
        promises.push(throttler.throttle(1).catch(() => {})); // Catch to avoid unhandled rejections
    }

    // Give a tiny delay to ensure all are processed
    await new Promise(resolve => setTimeout(resolve, 10));

    // 7th request should throw immediately (queue is full with 5 items)
    try {
        await throttler.throttle(1);
 ...
 */
async function testFullCapacityAtStartup(): Promise<void> {
 ...
 */
async function testDifferentIntervals(): Promise<void> {
    console.log('Test 10: Different interval types');

    const intervals = [
        { interval: 'second' as const, expectedMs: 1000 },
        { interval: 'minute' as const, expectedMs: 60 * 1000 },
        { interval: 'hour' as const, expectedMs: 60 * 60 * 1000 },
        { interval: 'day' as const, expectedMs: 24 * 60 * 60 * 1000 },
    ];
    ...
    for (const { interval, expectedMs } of intervals) {
        ...
        assert(
            limiter.getInfo().windowMs === expectedMs,
            `${interval} should be ${expectedMs}ms`
        );
    }
 ...
 */
async function testOKXSubAccountLimit(): Promise<void> {
 ...
 */
async function testPreciseTiming(): Promise<void> {
 ...
 * Run all tests
 */
async function testMultiWindowThrottler(): Promise<void> {
