    ...
    public virtual void handleOHLCV(WebSocketClient client, object message)
    {
        ...
        for (object i = 0; isLessThan(i, getArrayLength(marketIds)); postFixIncrement(ref i))
        {
            ...
            for (object j = 0; isLessThan(j, getArrayLength(timeframes)); postFixIncrement(ref j))
            {
                ...
                object symbol = getValue(market, "symbol");
                object stored = this.safeValue(getValue(this.ohlcvs, symbol), timeframe, new List<object>() {});
                object storedLength = getArrayLength(stored);
                if (isTrue(isGreaterThan(storedLength, 0)))
                {
                    object lastOhlcv = getValue(stored, subtract(storedLength, 1));
                    object ohlcvs = this.createStreamOHLCV(symbol, timeframe, lastOhlcv);
