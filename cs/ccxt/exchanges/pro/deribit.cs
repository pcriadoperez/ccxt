...
public partial class deribit : ccxt.deribit
{
    ...
    public virtual void handleOrders(WebSocketClient client, object message)
    {
        ...
        {
            this.streamProduce("orders", getValue(orders, i));
    ...
    public override void handleMessage(WebSocketClient client, object message)
    {
        ...
        {
            var err = new ExchangeError(add(add(this.id, " "), this.json(error)));
            this.streamProduce("errors", null, err);
            ((WebSocketClient)client).reject(err);
        }
        ...
        if (isTrue(!isEqual(channel, null)))
        {
            ...
            var err = new NotSupported(add(add(this.id, " no handler found for this message "), this.json(message)));
            this.streamProduce("errors", null, err);
            ((WebSocketClient)client).reject(err);
        }
