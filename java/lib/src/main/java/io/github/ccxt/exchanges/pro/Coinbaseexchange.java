   ...
   public Coinbaseexchange (Object options) {
    ...
    put( "sell", "buy" );
        ...
        if (Helpers.isTrue(Helpers.isTrue((!Helpers.isEqual(Helpers.GetValue(parsed, "cost"), null))) && Helpers.isTrue((!Helpers.isEqual(feeRate, null)))))
        {
            ...
            feeCost = Helpers.multiply(cost, feeRate);
        }
                ...
                if (Helpers.isTrue(Helpers.isTrue((Helpers.isEqual(previousSequence, null))) || Helpers.isTrue((Helpers.isGreaterThan(sequence, previousSequence)))))
                {
                    if (Helpers.isTrue(Helpers.isEqual(type, "match")))
                    {
                        ...
                        for (var i = 0; Helpers.isLessThan(i, Helpers.getArrayLength(trades)); i++)
                        {
                            ...
                            totalCost = this.sum(totalCost, Helpers.GetValue(tradeEntry, "cost"));
                            totalAmount = this.sum(totalAmount, Helpers.GetValue(tradeEntry, "amount"));
                        }
                        if (Helpers.isTrue(Helpers.isGreaterThan(totalAmount, 0)))
                        {
                            Helpers.addElementToObject(previousOrder, "average", Helpers.divide(totalCost, totalAmount));
                        }
                        ...
                        {
                            Helpers.addElementToObject(previousOrder, "filled", Helpers.add(((java.util.HashMap<String, Object>)previousOrder).get("filled"), Helpers.GetValue(trade, "amount")));
                            if (Helpers.isTrue(!Helpers.isEqual(Helpers.GetValue(previousOrder, "amount"), null)))
                            {
                                Helpers.addElementToObject(previousOrder, "remaining", Helpers.subtract(Helpers.GetValue(previousOrder, "amount"), Helpers.GetValue(previousOrder, "filled")));
                            }
                        }
    ...
    put( "currency", Helpers.GetValue(Helpers.GetValue(trade, "fee"), "currency") );
                    ...
                    } else if (Helpers.isTrue(Helpers.isTrue((Helpers.isEqual(type, "received"))) || Helpers.isTrue((Helpers.isEqual(type, "done")))))
                    {
                        ...
                        Object keys = new java.util.ArrayList<Object>(((java.util.Map<String, Object>)order).keySet());
    ...
    public Object parseWsOrder(Object order, Object... optionalArgs)
    {
        ...
        {
            filled = Helpers.subtract(amount, remaining);
