   ...
   public Bingx (Object options) {
        ...
        return java.util.concurrent.CompletableFuture.supplyAsync(() -> {
            ...
            } catch(Exception error)
            {
                ...
                for (var i = 0; Helpers.isLessThan(i, Helpers.getArrayLength(types)); i++)
                {
                    ...
                    Object url = Helpers.add(Helpers.add(Helpers.GetValue(Helpers.GetValue(Helpers.GetValue(this.urls, "api"), "ws"), type), "?listenKey="), listenKey);
                    Client client = this.client(url);
    ...
    public java.util.concurrent.CompletableFuture<Object> pong(Client client, Object message2)
    {
        ...
        return java.util.concurrent.CompletableFuture.supplyAsync(() -> {
            ...
            {
                var error = new NetworkError(Helpers.add(Helpers.add(this.id, " pong failed with error "), this.json(e)));
                client.reset(error);
            }
            return null;
        }, io.github.ccxt.Exchange.VIRTUAL_EXECUTOR);
